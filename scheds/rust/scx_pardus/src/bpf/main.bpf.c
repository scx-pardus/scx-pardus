...
/* Bu array herhangi bir anda çalışabilecek maksimum task sayısını gösterir.*/
#define PREDICTION_ARRAY_SIZE 4096

/* Kaç adet runtime slice ring bufferde kayıt edilecek*/
#define RUNTIME_HISTORY_SIZE 50
...
/*
 * Task için vdeadline hesaplama fonksiyonu
 *
...
static u64 task_dl(struct task_struct *p, s32 cpu, struct task_ctx *tctx) {
  ...
  u64 predicted_ml_timeslice = get_ml_timeslice(p->pid);
   ...
   */
  return p->scx.dsq_vtime + tctx->awake_vtime + predicted_ml_timeslice;
}
